    ...
    const int charNum = 9;
    char string[charNum] = {'I',3,'A','R','D','U','I','N','O'};
...
void sineWave3d(){
  ...
  for(int a = 0; a < 66; a++){
    for(int x = 0; x < 8; x++){
      for(int z = 0; z < 8; z++){
        rads = ((float)sqrt((x-4)*(x-4) + (z-4)*(z-4)) / 4.) * 2 + t; //change that 3 to make it shrink/expand
        t += 0.006; //time, change this to make it speed up/slow down
        while(t > TWO_PI) //limit to 2PI, wrap around if overflow
          t -= TWO_PI;
        while(rads > TWO_PI) //limit to 2PI, wrap around if overflow
          rads -= TWO_PI;
        clrvoxel(x, y[x][z] , z); //clear old LED
